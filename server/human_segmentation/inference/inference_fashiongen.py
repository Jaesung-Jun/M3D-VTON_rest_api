...
from inference_dataset import get_fashiongen_dataloader
#
import argparse
...
def inference(net, opts, use_gpu=True):
    ...
    inference_dataloader = get_fashiongen_dataloader('/data/wubowen/FashionGenAttnGAN/data/fashiongen', '/opt/ohpc/wubowen/fashiongen_parsing_result', phase='validation')
    sstime = time.time()
    ...
    for i_batch, data in enumerate(inference_dataloader):
        ...
        output_path = data['vis_output_path'][0]
        label_output_path = data['label_output_path'][0]
        # origin_image = data['image']
        # origin_output_path = data['origin_output_path']
        ...
        for epoch in range(1):
            ...
            # origin_image.save(origin_output_path)
