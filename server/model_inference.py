...
def run(model_type, uid):
    results = os.path.join("../user_requests", uid, "results")
    opt = TestOptions().parse()  # get test options
    opt.model = model_type
    opt.name = model_type
    opt.datalist = "test_pairs"
    opt.warproot = os.path.join("../user_requests", uid, "results", "aligned", "MTM", "test_pairs")
    opt.dataroot = os.path.join("../user_requests", uid)
    results_dir = os.path.join(results, opt.datamode, opt.name, opt.datalist)
    ...
    print(f'\nTesting {opt.model} finished and GPU memory cleared.')
