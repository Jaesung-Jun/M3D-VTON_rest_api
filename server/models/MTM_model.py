...
class MTMModel(BaseModel):
    ...
    def forward(self):
        ...
        if self.output['grid_tps'] is not None:
            ...
            #   Deprecated Settings Modified; (BEFORE)
            ...
            self.warped_cloth_mask = self.tps_cloth_mask = F.grid_sample(self.cm, self.grid_tps, padding_mode='zeros', align_corners=True)
            self.warped_cloth = self.tps_cloth = F.grid_sample(self.c, self.grid_tps, padding_mode='border', align_corners=True)
            self.tps_grid = F.grid_sample(self.im_g, self.grid_tps, padding_mode='zeros', align_corners=True)
