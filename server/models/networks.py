...
class DRM(nn.Module):
    ...
    def forward(self, input_data, inter_mode='bilinear'):
        ...
        x3u = nn.functional.interpolate(x3, size=x2.shape[2:4], mode=inter_mode, align_corners=True)
        x3u = self.l3u(torch.cat((x3u, x2), dim=1))
        #x2u = nn.functional.interpolate(x3u, size=x1.shape[2:4], mode=inter_mode)
        x2u = nn.functional.interpolate(x3u, size=x1.shape[2:4], mode=inter_mode, align_corners=True)
        x2u = self.l2u(torch.cat((x2u, x1), dim=1))
        #x1u = nn.functional.interpolate(x2u, size=x0.shape[2:4], mode=inter_mode)
        x1u = nn.functional.interpolate(x2u, size=x0.shape[2:4], mode=inter_mode, align_corners=True)
        x1u = self.l1u(torch.cat((x1u, x0), dim=1))
        #x0u = nn.functional.interpolate(x1u, size=input_data.shape[2:4], mode=inter_mode)
        x0u = nn.functional.interpolate(x1u, size=input_data.shape[2:4], mode=inter_mode, align_corners=True)
        x0u = self.l0u(x0u)
        return x0u
