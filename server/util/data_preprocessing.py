...
def preprocess_data(MPV3D_root):
    ...
    for data_mode in data_modes:
        ...
        for i, imname in tqdm(enumerate(p_names)):
            ...
            #parse = ImageOps.grayscale(parse)
            parse_array = np.array(parse)
            """
            print(parse_pth)
            if "BJ" in parse_pth:
                for i in range(len(parse_array)):
                    for j in range(len(parse_array[i])):
                        if parse_array[i][j] == 15:
                            print("[x:",j," y:",i,"]", end="")
                            print(parse_array[i][j], end="")
                            break
            """
            #print(str(parse_array[254][174]))
            # 14 -> 왼팔
            # 15 -> 오른팔
            # 5 -> 셔츠
