...
def run(results_dir):
    ...
    """
    depth_root = os.path.join(results_dir, "results", "aligned", "DRM", "test_pairs", 'final-depth')
    frgb_root = os.path.join(results_dir, "results", "aligned", "TFM", "test_pairs", 'tryon')
    parse_root = os.path.join(results_dir, 'image-parse')
    point_dst = os.path.join(results_dir, 'results')
    depth_list = sorted(os.listdir(depth_root))

    """
    depth_root = 'results/aligned/DRM/test_pairs/final-depth'
    frgb_root = 'results/aligned/TFM/test_pairs/tryon'
    parse_root = 'mpv3d_example/image-parse'
    point_dst = 'results/aligned/pcd/test_pairs'
    depth_list = sorted(os.listdir(depth_root))
    """
